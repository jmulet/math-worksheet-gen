...
export function wsExporterLatex(adt: AbstractDocumentTree, opts: any): string {

    const builder = ["# Generated by wsMath on " + new Date(), PREAMBLE];
    ...
    return processMarkdown(builder.join("\n"));    
}
...
function renderActivity(activity: ActivityTree, opts: any): string[] {
    ...
    if (activity.questions.length === 0) {
        ...
        activity.formulation.replace(/\n/g, '\n\n')  + 
        "}}\n\\vspace{0.25cm}\n\n");

    } else if (activity.questions.length === 1) {
        activity.questions.forEach((question) => {
            latex.push("    \\item " + activity.formulation.replace(/\n/g, '\n\n') + question.formulation);
        });           
    } else { 
        latex.push("    \\item " + activity.formulation.replace(/\n/g, '\n\n'));
        ...
        activity.questions.forEach((question, i) => {
            const questionLaTeX = question.formulation;
        ...
        if (Math.abs(opts.includeKeys)===1) {
            if (i === 0 ) {
                if (opts.includeKeys === -1) {
                    latex.push("      \\task " + question.steps);
                } else {
                    latex.push("      \\task " + question.answer);
                }
            }
        } else {
...
function processMarkdown(mdw: string): string {

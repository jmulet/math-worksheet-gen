...
export async function wsExporterLatex(adt: AbstractDocumentTree, opts: any): Promise<string> {
    ...
    opts.toFormat = opts.toFormat || "pdf";
    ...
    adt.graphics.forEach((g) => {
        if (g.base64) {
            builder.push("\\begin{filecontents*}{"+g.id+"." + opts.toFormat + "64}");
    ...
    adt.graphics.forEach((g) => {
        if (g.base64) {
            builder.push("\\immediate\\write18{base64 -D " + g.id +"."+ opts.toFormat+"64 > " + g.id + "." + opts.toFormat+"}");        
        }
    });
    builder.push("%");

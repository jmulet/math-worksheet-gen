        ...
        if (Math.abs(opts.includeKeys)===1) {
            if (i === 0 ) {
                if (opts.includeKeys === -1) {
                    latex.push(startLi + (markdownToLatex(question.steps || question.answer) || i18n("NO_ANSWER", opts.lang)) );
                } else {
                    latex.push(startLi +  (markdownToLatex(question.answer) || i18n("NO_ANSWER", opts.lang)) );
                }
            }
        } else {
...
function markdownToLatex(str: string): string {
    if (!str) {
        return str;
    }

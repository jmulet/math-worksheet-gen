...
function wsExporterHtml(adt, opts) {
    const PREAMBLE = `
<!DOCTYPE html>
<html>
<head>
...
`;
    ...
    builder.push(`<p class=\"arial\"> <b>${wsExporter_1.i18n('REFERENCE', opts.lang)}</b>   ${adt.sid}  /  ${adt.seed} . <b>${wsExporter_1.i18n('NAME', opts.lang)}:</b>  
    ...
    if (adt.sections) {
        adt.sections.forEach((section, sectionIndx) => {
            builder.push(" <br/><h3>" + roman_numerals_1.toRoman(sectionIndx + 1) + ". " + section.title + "</h3>");
            builder.push('<ol start="' + activityCounter + '">');
    ...
    if (opts.includeKeys !== 0 && !opts.keysPlacement) {
        builder.push(`  <hr/><h3><b>${wsExporter_1.i18n('ANSWERS', opts.lang)}</b></h3>`);
        builder.push("  <ol>");
        if (adt.activities) {
            adt.activities.forEach(activity => {
            ...
            if (Math.abs(opts.includeKeys) === 1) {
                if (i === 0) {
                    if (opts.includeKeys === -1) {
                        html.push(startLi + (markdownToHtml(question.steps || question.answer)) || wsExporter_1.i18n("NO_ANSWER", opts.lang)) + endLi;
                    }
                    else {
                        html.push(startLi + (markdownToHtml(question.answer)) || wsExporter_1.i18n("NO_ANSWER", opts.lang)) + "</li>";
                    }
                }
            }
            else {
                if (opts.includeKeys === -2) {
                    ...
                    html.push(startLi + (markdownToHtml(question.answer)) || wsExporter_1.i18n("NO_ANSWER", opts.lang)) + endLi;
                }
            }
        });
        if (nquestions > 1) {
...
exports.wsExporterHtml = wsExporterHtml;
...
function filterHtml(src) {
    return src;
}
//# sourceMappingURL=wsExporterHtml.js.map